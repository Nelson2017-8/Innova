...
class SubcategoriaController extends Controller
{
	...
	private $data = [
		'titleForm' => 'Mostrar Subcategorias',
		'linkRegistro' => true,
		'linkEliminar' => true,
		'linkActualizar' => true,
		...
		'edit' => true,
		'Card' => [
			'titleCard' => 'Mostrar todas las Subcategorias',
	...
	public function index(Request $request)
	{
		...
		$data = $this->data;
		$inputsTable = [];
		if ( isset(Subcategoria::All()[0]) ){
			$inputsTable = Subcategoria::All()[0]->getFillable();
		}else{
		...
		if (!isset($noHayRegistro)){
			...
			$tabla = ($tabla == null) ? Subcategoria::orderBy('updated_at', 'desc')
			...
			// RELACION CATEGORIA
			foreach ($tabla as $indice => $valor) {
				$tabla[$indice]->_categoria = $valor->categoria->nombre;
			}
    ...
    {
		$forms = $this->form;
		$forms['action'] = route('subcategoria.store');
		$forms['btn']['title'] = 'Registrar';
		$data = $this->data;
		$data['relations'] = true;
		$data['Relacionado'][0] = 'show subcategoria';
		$data['Relacionado'][1] = 'show categoria';
		$data['Relacionado'][2] = 'register categoria';
		$title = 'Registrar Subcategoria';

		// DAR VALORES AL SELECT
		$categorias = Categoria::orderBy('nombre')->select('id', 'nombre')->get();
		foreach ($categorias as $categoria) {
			$forms['select'][0]['option'][$categoria->id] = $categoria->nombre;
		}
    ...
    {
		$subcategorium = new Subcategoria($request->all());
		$subcategorium->categoria_id = $request->categoria_id;
		$subcategorium->save();

		$respuesta = ['messenger' => 'La subcategoria ha sido registrado correctamente', 'result' => 'success'];
    ...
    {
		$forms = $this->form;
		$forms['action'] = route('subcategoria.update', $subcategorium->id);
		$forms['data-update'] = route('subcategoria.index');
		...
		// valores de la DB
		$forms['input'][0]['attr']['data-value'] = $subcategorium->nombre;
		$forms['textarea'][0]['attr']['data-value'] = $subcategorium->descripcion;
		$forms['select'][0]['attr']['data-value'] = $subcategorium->categoria_id;
		$data = $this->data;
		$data['relations'] = true;
		$data['Relacionado'][2] = 'show subcategoria';
		$data['Relacionado'][0] = 'register subcategoria';
		$data['Relacionado'][1] = 'register categoria';

		$title = 'Actualizar "' . $subcategorium->nombre. '"';
    ...
    public function update(Request $request, Subcategoria $subcategorium)
    {
		...
		$subcategorium->descripcion = $request->descripcion;
		$subcategorium->categoria_id = $request->categoria_id;

		// GUARDO
		$subcategorium->update();
		$messenger = 'La subcategoria "' . $subcategorium->nombre . '" ha sido actualizado';
    ...
    {
		$messenger = 'La subcategoria "' . $subcategorium->nombre. '" ha sido eliminado';
